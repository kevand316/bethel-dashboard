...
- **Sessions persist in the SDK's default storage. Do not roll your own.** The Supabase JS SDK persists sessions in localStorage under its own keys. Do not store auth tokens, JWTs, or user IDs in custom localStorage keys. Read identity exclusively via `supabase.auth.getUser()` or `getSession()`.
  ...
  **One documented exception: the Google Drive access token** (`lib/drive.js`, key `bethel.drive.<userId>`). This rule is about Supabase identity, which the SDK owns. A Google credential is a different credential for a different service and the Supabase SDK cannot hold it. It is stored deliberately, and removing it is a regression, not a cleanup — without it every page reload sends the operator back to a Connect button, because Google's token request opens a popup and browsers block popups that were not triggered by a click. Operator chose this over per-tab storage on 2026-08-25, for the whole-product user experience. Guards that make it acceptable, all of which must stay: scope is `drive.file` only, so the token reaches nothing but files this app created; it carries an expiry and is discarded once past it; it is cleared on Drive disconnect, on any 401 from Drive, and on dashboard sign-out. Covered by tests 17 and 18 in `tests/intake.spec.js`.
