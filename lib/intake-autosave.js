...
//     the write and returns a new revision, and at no other time.
...
(function () {
  ...
  let session = null; // { fileId, revision, getRecord }
  let statusCb = null;
  ...
  const STATUS_TEXT = {
    ...
    conflict: ["offline", "NOT SAVED YET"],
  ...
  async function doSave() {
    ...
    try {
      const { revision } = await window.drive.saveIntake(session.fileId, record, session.revision);
      ...
      if (e.code === "conflict") {
        ...
        // the operator decides whose revision wins.
  ...
  // ── public surface ─────────────────────────────────────────────────────────
  function attach({ fileId, revision, getRecord, onStatus }) {
    clearTimers();
    session = { fileId, revision, getRecord };
  ...
  // The operator chose to overwrite whatever is in Drive. Drop the revision check
  // for one save, then resume normal conflict detection with the new revision.
