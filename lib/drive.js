...
(function () {
  ...
  //
  // PLACEHOLDER_CLIENT_ID stays as the "not set up yet" sentinel and must never
  // be given a real value — isConfigured() decides whether the Intake tab works
  // by comparing against it, so setting it to the live ID would make that check
  // compare the ID to itself and report the tab as unconfigured forever.
  const PLACEHOLDER_CLIENT_ID = "PASTE_YOUR_GOOGLE_OAUTH_CLIENT_ID";
  const CONFIGURED_CLIENT_ID =
    "909342292895-e2498svuhe2lvce2qhjaq7fqh8p6e52t.apps.googleusercontent.com";
  const GOOGLE_CLIENT_ID = window.BETHEL_GOOGLE_CLIENT_ID || CONFIGURED_CLIENT_ID;
  ...
  // ── connection hint ────────────────────────────────────────────────────────
  // What we keep, per dashboard user: which Google account they connected, and
  // the current access token with its expiry. Never client data — this is the
  // operator's own email and their own credential for their own Drive.
  //
  // Persisting the token is what makes a page reload keep working. The original
  // version stored only the email and re-asked Google silently on load, and that
  // failed every single time in practice: Google's token request opens a popup,
  // and browsers block popups that aren't triggered by a click. So every reload
  // dropped the operator back to a Connect button. Reusing a token we already
  // hold needs no popup, no network call to Google, and no click.
  //
  // The trade-off, stated plainly: an access token sits in localStorage until it
  // expires, about an hour. Anyone with the unlocked machine could read it in
  // that window and reach the files this app created — which are the intake
  // records. It is scoped to drive.file, so it reaches nothing else in Drive.
  // This is the same posture as the Supabase session token the SDK already keeps
  // there, and it is cleared on disconnect and on sign-out.
  function readHint() {
  ...
  // Saves the live token alongside the remembered account, so the next page load
  // can pick it straight back up.
  function persistToken() {
    const hint = readHint() || {};
    writeHint({
      ...hint,
      email: account?.email || hint.email || "",
      name: account?.name || hint.name || "",
      token: accessToken,
      expiresAt: tokenExpiresAt,
    });
  }

  // Drops the credential but keeps the remembered email, so a returning operator
  // still sees "Reconnect as you@example.com" rather than a bare Connect button.
  function forgetToken() {
    const hint = readHint();
    if (!hint) return;
    delete hint.token;
    delete hint.expiresAt;
    writeHint(hint);
  }

  // Puts a stored token back into memory if it is still good. Returns false when
  // there is nothing usable, in which case the caller falls back to asking
  // Google — which needs a click.
  function restoreToken() {
    const hint = readHint();
    if (!hint?.token || !hint.expiresAt) return false;
    if (Date.now() >= hint.expiresAt - EXPIRY_SKEW_MS) return false;
    accessToken = hint.token;
    tokenExpiresAt = hint.expiresAt;
    return true;
  }
  ...
  function requestToken(interactive) {
    return new Promise(async (resolve, reject) => {
      ...
      client.callback = (resp) => {
        ...
        // Store it immediately. If the tab is reloaded a second later, that
        // reload must not have to ask Google again.
        persistToken();
        resolve(accessToken);
      };
  ...
  async function connect() {
    ...
    // persistToken, not writeHint: a bare writeHint here would drop the token
    // requestToken just stored and put us straight back to re-connecting on
    // every reload.
    persistToken();
    return account;
  }
  ...
  async function resume() {
    ...
    // First choice: a token we already hold. No popup, no round trip to Google's
    // sign-in, nothing for the operator to click. This is the path that makes an
    // ordinary page reload keep working.
    if (restoreToken()) {
      try {
        account = await whoami(); // cheap call that proves the token still works
        await ensureRootFolder();
        persistToken();
        return account;
      } catch {
        // Stored but no longer accepted — revoked from the Google account page,
        // or the grant was withdrawn. Drop it and try the slow path.
        accessToken = null;
        tokenExpiresAt = 0;
        forgetToken();
      }
    ...
    // Fallback: ask Google silently. Often blocked, because this runs on page
    // load rather than off a click and Google's token request opens a popup.
    // Worth attempting — when it works the operator never sees the gate — but
    // never relied upon.
    try {
      ...
      // Consent still on file, but nothing usable right now. Show the remembered
      // email next to a Reconnect button rather than a bare Connect, so the user
      // knows which account this dashboard expects.
      account = null;
      return null;
    }
  }
  ...
  // Our conflict token is headRevisionId — the id of the file's latest CONTENT
  // revision — and deliberately not `version`.
  //
  // `version` was the obvious choice and it was wrong. Drive documents it as
  // reflecting "every change made to the file on the server, even those not
  // visible to the user": our own metadata rename after each save, plus Google's
  // own indexing and housekeeping, whenever it feels like it. A single operator
  // on a single laptop would type, save, and be told their intake had been
  // "changed somewhere else" — by nobody. A conflict warning that cries wolf is
  // worse than none, because it trains people to click through the one that
  // matters.
  //
  // headRevisionId changes only when the bytes change. Renaming the file does
  // not move it; Drive re-indexing does not move it; another device writing the
  // record does. That is exactly the question we are asking.
  //
  // It is ALWAYS read from files.get and never from an upload response — the
  ...
  async function readRevision(fileId) {
    const meta = await api("/files/" + fileId, { query: { fields: "headRevisionId" } });
    return meta?.headRevisionId || null;
  }
  ...
  async function createIntake(record) {
    ...
    return { fileId, revision: await readRevision(fileId) };
  }
  ...
  async function readIntake(fileId) {
    ...
    return { record, revision: await readRevision(fileId) };
  }

  // Saves content, then metadata, then returns the new revision.
  // expectedRevision is what the caller last saw. If Drive disagrees, someone
  ...
  async function saveIntake(fileId, record, expectedRevision) {
    if (expectedRevision != null) {
      const current = await readRevision(fileId);
      if (current != null && String(current) !== String(expectedRevision)) {
    ...
    return { revision: await readRevision(fileId) };
  }
  ...
  window.drive = {
    ...
    resume,
    disconnect,
    // Signing out of the dashboard must not leave a live Drive credential behind
    // on a shared machine. Forgets the token but keeps the remembered email, so
    // signing back in is one click rather than a fresh consent screen.
    forgetToken,
    listIntakes,
    createIntake,
    readIntake,
