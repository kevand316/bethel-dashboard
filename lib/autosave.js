...
(function () {
    ...
    try {
      ...
      if (_loadedAt) {
        ...
        const { data: rows, error } = await client
          .from("bethel_data")
          .update({ data: entry.data, updated_at: newTs })
          .eq("id", entry.row_id)
      ...
      const { data: rows, error } = await client
        .from("bethel_data")
        .upsert(
          { id: entry.row_id, user_id: entry.user_id, data: entry.data, updated_at: newTs },
  ...
  async function executeWrite(entry) {
    ...
    if (result === "conflict") {
      // The conflict may be a false positive caused by our own flush() sending
      // a keepalive that changed updated_at without us tracking the new value.
      // Retry ONCE with an unconditional upsert before surfacing the banner.
      _loadedAt = null;
      ...
      // Still failing after unconditional upsert — genuine concurrent write.
  ...
  function flush() {
    ...
    // The keepalive POST will trigger the set_updated_at trigger in Postgres,
    // changing updated_at in the DB to a value we never see (keepalive is
    // fire-and-forget). Clear _loadedAt so the next in-page executeWrite uses
    // an unconditional upsert rather than a conditional UPDATE that would fail
    // because our stale _loadedAt no longer matches the DB value.
    _loadedAt = null;
  ...
  window.autosave = { init, push, flush, drainQueueOnLoad, setLoadedAt, overrideAndSave, getPendingData };
})();
