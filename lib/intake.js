...
(function () {
  ...
  async function openIntake(fileId) {
    setNotice("");
    try {
      const { record, revision } = await window.drive.readIntake(fileId);
      openFileId = fileId;
      openRecord = record;
      ...
      mountForm(record);
      window.intakeSave.attach({ fileId, revision, getRecord: currentRecord, onStatus });
      show("form");
    } catch (e) {
      if (e.code === "needs_reconnect") return renderGate();
  ...
  async function newIntake() {
    ...
    try {
      ...
      const { fileId, revision } = await window.drive.createIntake(record);
      openFileId = fileId;
      openRecord = record;
      mountForm(record);
      window.intakeSave.attach({ fileId, revision, getRecord: currentRecord, onStatus });
